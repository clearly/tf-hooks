  ...
  for( let type of _.keys( params.apply.add ) ) {
    ...
    if( types.indexOf( type ) > -1 ) {
      ...
      for( let action of actions ) {
        ...
        results = results.concat( yield instanceNames.map( instanceName => {
          let instance = _.get( params.state.resource, `${type}.${instanceName}` );
          return runHook( action, type, instanceName, instance, config );
        }) );
      }
    }
  }
...
function *runHook( action, type, name, instance, config ) {
  const body = { action, type, name, config : instance };
  let response = null;
  let result = null;
  try {
    ...
    if( response.statusCode == 200 ) {
      ...
      result = { valid : 'fail', detail : response.statusMessage };
    }
  } catch( err ) {
